...
#
#           V_12
#           This version should now incooperate the stejskal tanner equation
#           match the radibutton combo to a variable
#           
#   
###############################################################################
...
def stejskaltanner_gpz6_1H(gpz6_1H_val, D):
    d20 = get_d20_entry()
...
def GENERATE_FIT():
    ...
    initial_guess = inguess_entry  # Convert input string to a list or array
    if current_combo in ["gpz6_1H"]:
        ...
        try:
            ...
            popt, pcov = curve_fit(model_func, x_data, y_data, initial_guess)
            D_gpz6_1H = popt
            D_gpz6_1H_cov = pcov

            # Update your UI or store results as needed
            text_output_results.delete("1.0", tk.END)
            text_output_results.insert(tk.END, f"Fit result for {current_combo}: D = {D_gpz6_1H:.5e} DCov = {D_gpz6_1H_cov:.5e}")
